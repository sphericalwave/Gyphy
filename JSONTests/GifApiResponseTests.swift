...
class GifApiResponseTests: XCTestCase
{
    let decoder = JSONDecoder()
    let encoder = JSONEncoder()
    var gifResponse: GyphyResponse!
    //TODO: too complex build a fake to test against is obnoxious in this case...might be worthwhile as a exercise
//    let gifResponse = GyphyAPIResponse(gifs: [Gif(type: "gif"), Gif(type: "gif")], pagination: Pagination(totalCount: 90290, count: 1, offset: 0)
//, meta: Meta(status: 200, msg: "OK", responseId: "4837e2a4150ac37fb3157bcef8c9421364b1907d"))
//    let gifResponse = GyphyAPIResponse(gifs: [Gif(url: URL(string: "https://giphy.com/gifs/last-week-tonight-with-john-oliver-fake-paws-real-animals-dCBZcE46p8Yqj7RA4P")!), (url: URL(string: "https://giphy.com/gifs/last-week-tonight-with-john-oliver-fake-paws-real-animals-dCBZcE46p8Yqj7RA4P")!)], pagination: Pagination(totalCount: 90290, count: 1, offset: 0)
//    , meta: Meta(status: 200, msg: "OK", responseId: "4837e2a4150ac37fb3157bcef8c9421364b1907d"))
    ...
    override func setUp() {
        ...
        let url1 = URL(string:  "https://media2.giphy.com/media/dCBZcE46p8Yqj7RA4P/giphy.gif?cid=ff71a59d310c2923f92aa4770251539be5aab4a335e78363&rid=giphy.gif")!
        let oImage1 = OriginalImage(url: url1)
        let images1 = Images(original: oImage1)
        let gif1 = Gif(images: images1, id: "dCBZcE46p8Yqj7RA4P") //(images: images1)
        ...
        self.gifResponse = GyphyResponse(gifs: [gif1], pagination: pagination, meta: meta)
    }
    
    func testEncodeDecode() {
        guard let data = try? encoder.encode(gifResponse) else { fatalError() }
        XCTAssertNotNil(data)
        guard let zGifData = try? decoder.decode(GyphyResponse.self, from: data) else { fatalError() }
        XCTAssertEqual(zGifData, gifResponse)
    }
    ...
    func testSerialization() {
        ...
        do {
            let zGifData = try decoder.decode(GyphyResponse.self, from: jsonData)
            XCTAssertNotNil(zGifData.gifs[0].images.original.url)
            XCTAssertNotNil(zGifData.meta.msg)
            XCTAssertNotNil(zGifData.pagination.count)
            XCTAssertNotNil(zGifData) //TODO:
        }
        catch {
